...
{ Style paint patch based on jvToolEdit under MPL       }
{   Radek Cervinka                                      }
{     where Contributers:                               }
...
begin
  ...
  if csDestroying in Editor.ComponentState then
    ...
    if StandardPaint and not (csPaintCopy in Editor.ControlState)  then
    begin
    ...
    with ACanvas do
    begin
      SendRectMessage(Editor.Handle, EM_GETRECT, 0, R);
      if not Editor.Ctl3D and (Editor.BorderStyle = bsSingle) then
      begin
        Brush.Color := clWindowFrame;
        FrameRect(Editor.ClientRect);
      end;
        S := AText;
        AWidth := TextWidth(S);
        if Editor.PopupVisible then
          ALeft := R.Left
        else
        begin
          case AAlignment of
            ...
            taRightJustify:
              ...
              ALeft := (R.Right + R.Left - AWidth) div 2;
          end;
        end;
      ...
      begin
        ...
        (Editor).Perform(WM_ERASEBKGND, ACanvas.Handle, 0);
        ...
        try
          ...
          ACanvas.TextRect(R, ALeft, R.Top, S);
        finally
...
var
  Loc: TRect;
begin
  if not Ctl3D then //add JB.14.01.2025
    SetRect(Loc, 2, 0, ClientWidth - FBtnControl.Width, ClientHeight + 1)    {@}
  else
